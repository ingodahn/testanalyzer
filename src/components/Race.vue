<template>
    <div id="race">
        ...
        <!--
        <p>{{ ScoredSorted.length }} Studenten</p>
        -->

        <svg v-if="ScoredSorted.length > 0" id="racetrack" :width="width" :height="height" >
            <line :x1="width-left" :y1="top" :x2="width-right" :y2="top" stroke="black" stroke-width="1px"/>
            <line :x1="width-left" :y1="bottom" :x2="width-right" :y2="bottom" stroke="black" stroke-width="1px"/>
            <line :x1="width-left" :y1="top" :x2="width-left" :y2="bottom" stroke="black" stroke-dasharray="5 5" stroke-width="1px"/>
            <image xlink:href="../../public/flag.svg" :x="width-60" y="100" width="70" height="70"/>
            ...
            <text :x=width-right-10 :y=bottom+15 fill="black">0 %</text>
                ...
                :cx= item.x :cy=item.y :r="diameter"
    ...
    computed: {
        studentScores: function () {
            ...
            for (var i=0; i<this.ScoredSorted.length; i++) {
                ...
                studentScores.push({name: s.name, x: this.width-this.right+diff*s.totalScore/this.TotalScore,
                y: this.top+this.diameter+Math.random()*(this.bottom-this.top-2*this.diameter)
                });
            }
        ...
        trackMsg: function () {
            if (this.curStudent == "none") {
                return this.ScoredSorted.length + " Studierende"
            } else {
